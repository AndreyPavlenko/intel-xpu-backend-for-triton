#ifndef TRITON_CONVERSION_TRITON_GPU_TO_LLVM_PISA_ASM_FORMAT_H_
#define TRITON_CONVERSION_TRITON_GPU_TO_LLVM_PISA_ASM_FORMAT_H_
...
struct pISAInstr;
struct pISAInstrCommon;
struct pISAInstrExecution;

// pISABuilder helps to manage a pISA asm program consists of one or multiple
// instructions.
//
// A helper for building an ASM program, the objective of pISABuilder is to give
...
// builder.dump()
//
// To get all the mlir::Value used in the pISA code,
//
...
//
// pISABuilder can build a pISA asm with multiple instructions, sample code:
//
// pISABuilder builder;
...
// mov(...);
// cp(...);
// This will get a pISA code with two instructions.
//
// Similar to a C function, a declared pISAInstr instance can be launched
...
//
// Finally, we will get a pISA code with two mov instructions.
//
// There are several derived instruction type for typical instructions, for
// example, the pISAIOInstr for ld and st instructions.
struct pISABuilder {
  struct Operand {
  ...
  template <typename INSTR = pISAInstr, typename... Args>
  ...
  // branch but not both, the register is undefined and pISAas can perform
...
private:
  ...
  void reorderArgArchive(ArrayRef<Operand *> order) {
    ...
    // determined by pISA code snippet passed from external.
  ...
  friend struct pISAInstr;
  friend struct pISAInstrCommon;

protected:
  llvm::SmallVector<std::unique_ptr<Operand>, 6> argArchive;
  llvm::SmallVector<std::unique_ptr<pISAInstrCommon>, 2> instrs;
  llvm::SmallVector<std::unique_ptr<pISAInstrExecution>, 4> executions;
  int oprCounter{};
};

// pISA instruction common interface.
// Put the generic logic for all the instructions here.
struct pISAInstrCommon {
  explicit pISAInstrCommon(pISABuilder *builder) : builder(builder) {}

  using Operand = pISABuilder::Operand;

  // clang-format off
  pISAInstrExecution& operator()() { return this->call({}); }
  pISAInstrExecution& operator()(Operand* a) { return this->call({a}); }
  pISAInstrExecution& operator()(Operand* a, Operand* b) { return this->call({a, b}); }
  pISAInstrExecution& operator()(Operand* a, Operand* b, Operand* c) { return this->call({a, b, c}); }
  pISAInstrExecution& operator()(Operand* a, Operand* b, Operand* c, Operand* d) { return this->call({a, b, c, d}); }
  pISAInstrExecution& operator()(Operand* a, Operand* b, Operand* c, Operand* d, Operand * e) { return this->call({a, b, c, d, e}); }
  pISAInstrExecution& operator()(Operand* a, Operand* b, Operand* c, Operand* d, Operand * e, Operand* f) { return this->call({a, b, c, d, e, f}); }
  pISAInstrExecution& operator()(Operand* a, Operand* b, Operand* c, Operand* d, Operand * e, Operand* f, Operand* g) { return this->call({a, b, c, d, e, f, g}); }
  // clang-format on

  // Set operands of this instruction.
  pISAInstrExecution &operator()(llvm::ArrayRef<Operand *> oprs,
...
protected:
  ...
  // pISA code.
  pISAInstrExecution &call(llvm::ArrayRef<Operand *> oprs,
                           bool onlyAttachMLIRArgs = false);

  pISABuilder *builder{};
  llvm::SmallVector<std::string, 4> instrParts;

  friend struct pISAInstrExecution;
};

template <class ConcreteT> struct pISAInstrBase : public pISAInstrCommon {
  using Operand = pISABuilder::Operand;

  explicit pISAInstrBase(pISABuilder *builder, const std::string &name)
      : pISAInstrCommon(builder) {
    o(name);
  }

  // Append a suffix to the instruction.
  // e.g. pISAInstr("add").o("s32") get a add.s32.
  // A predicate is used to tell whether to apply the suffix, so that no if-else
  // code needed. e.g. `pISAInstr("add").o("s32", isS32).o("u32", !isS32);` will
...
struct pISAInstr : public pISAInstrBase<pISAInstr> {
  using pISAInstrBase<pISAInstr>::pISAInstrBase;
  ...
  pISAInstr &v(int vecWidth, bool predicate = true);

  // Append a".b[0-9]+" to the instruction
  pISAInstr &b(int width);
};

// Record the operands and context for "launching" a pISAInstr.
struct pISAInstrExecution {
  using Operand = pISABuilder::Operand;

  llvm::SmallVector<Operand *> argsInOrder;

  pISAInstrExecution() = default;
  explicit pISAInstrExecution(pISAInstrCommon *instr,
  ...
  pISAInstrExecution &predicate(mlir::Value value, StringRef constraint = "b") {
    pred = instr->builder->newOperand(value, constraint);
    return *this;
  }

  // Prefix a predicate to the instruction, if non-null
  pISAInstrExecution &maybePredicate(mlir::Value value,
  ...
  // Prefix a !predicate to the instruction.
  pISAInstrExecution &predicateNot(mlir::Value value, StringRef constraint) {
    pred = instr->builder->newOperand(value, constraint);
...
#endif // TRITON_CONVERSION_TRITON_GPU_TO_LLVM_PISA_ASM_FORMAT_H_
